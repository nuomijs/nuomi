...
var _nuomi = _interopRequireDefault(require("../core/nuomi"));
...
var BaseNuomi = /*#__PURE__*/function (_React$PureComponent) {
  ...
  _createClass(BaseNuomi, [{
    ...
    value: function getId() {
      ...
      if (!id) {
        return "@@nuomi_".concat(++BaseNuomi.nuomiId);
      }

      if ((0, _store.getStore)(id)) {
        throw new Error("storeId\uFF1A".concat(id, " \u5DF2\u88AB\u5B9A\u4E49\uFF0C\u8BF7\u91CD\u65B0\u547D\u540D\uFF01"));
      }
    ...
    value: function createStore() {
      ...
      store.dispatch = /*#__PURE__*/function () {
        var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(action) {
          var type, payload, effects, splitIndex, loadingQueue, loadingType, effectsProxy, loadingPayload, lastEffect, id, effect, $store, dispatchReturn;
            ...
            while (1) {
              switch (_context.prev = _context.next) {
                ...
                case 3:
                  ...
                  effects = _this3.effects; // type中包含斜杠视为调用其他模块方法
                  ...
                  if (!((0, _utils.isObject)(effects) && (0, _utils.isFunction)(effects[type]))) {
                    _context.next = 26;
                    break;
                  }
                  ...
                  // 带有loading功能的方法队列
                  loadingQueue = [];
                  loadingType = "".concat(store.id, "/_updateLoading");
                  _context.prev = 10;
                  // 通过代理可以知道调用的方法内部调用情况，调用的函数本身以及函数内部调用的方法或者属性都会走get
                  effectsProxy = new _effectsProxy.default(effects, {
                    // name是当前调用的方法或者属性名
                    get: function get(target, name) {
                      var effect = effects[name]; // $开头的方法进行loading特殊处理
    ...
    value: function createReducer() {
      var _this$props2 = this.props,
          ...
          stateData = _this$props2.state,
          reducers = _this$props2.reducers;
      var defaultState = (_globalWindow.default[_store.INITIALISE_STATE] || _store.initialiseState || {})[store.id];

      if (defaultState) {
        defaultState = _nuomi.default.extend({
          state: stateData
        }, {
          state: defaultState
        }).state;
      ...
      (0, _reducer.createReducer)(store.id, function () {
        ...
        if (type.indexOf(typePrefix) === 0) {
          ...
          (0, _warning.default)(false, "\u672A\u5B9A\u4E49actionType\u4E3A ".concat(type, " \u7684 reducer\uFF0C\u5982\u679C\u4F60\u60F3\u8C03\u7528 effects \u4E2D\u7684\u65B9\u6CD5\uFF0C\u8BF7\u4F7F\u7528\n          \nstore.getStore('").concat(store.id, "').dispatch({\n  type: '").concat(key, "',\n  payload,\n})"));
        }
    ...
    value: function render() {
      var props = this.props;
      var children = props.render ? props.render(props.children) : props.children;
...
exports.default = BaseNuomi;

_defineProperty(BaseNuomi, "propTypes", _propTypes.NuomiPropTypes);
