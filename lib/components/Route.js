...
function (_React$PureComponent) {
  ...
  _createClass(Route, [{
    ...
    value: function componentWillUnmount() {
      (0, _reducer.removeReducer)(this.store.id); // 嵌套路由时防止子路由被销毁后再创建无法匹配问题
    ...
    value: function render() {
      ...
      return _react.default.createElement(_Context.RouterContext.Consumer, null, function (context) {
        ...
        var matchLocation = _router.default.matchPath(location, path);

        var match = matchLocation !== false; // context.matched 表示同一个上下文中，多个路由只匹配一个

        if (!match || context.matched && context.matched !== _this2) {
          // 设置了cache没有匹配路由，不销毁，只隐藏
          if (cache === true && _this2.routeComponent !== null) {
